...
// Article -> ontology references, generated by scripts/build-ontology-crosslinks.mjs
// from `> **Ontology:** T1, MC4` lines in article frontmatter. Validated against
// public/data/atlas_nodes.json. Articles without an Ontology line simply do not
// emit a footer; this map is keyed by slug and may be empty.
const ARTICLE_ONTOLOGY_PATH = path.join(PUBLIC_ROOT, 'data', 'article_ontology.json');
const ARTICLE_ONTOLOGY = JSON.parse(fs.readFileSync(ARTICLE_ONTOLOGY_PATH, 'utf8'));
if (!ARTICLE_ONTOLOGY || typeof ARTICLE_ONTOLOGY.by_article !== 'object' || ARTICLE_ONTOLOGY.by_article === null) {
  throw new Error(`Invalid article_ontology.json: missing .by_article object`);
}

for (const key of ['total_articles_with_links', 'total_links']) {
  if (!Number.isSafeInteger(ARTICLE_ONTOLOGY[key]) || ARTICLE_ONTOLOGY[key] < 0) {
    throw new Error(`Invalid article_ontology.json: ${key} must be a non-negative integer`);
  }
}

for (const [slug, uris] of Object.entries(ARTICLE_ONTOLOGY.by_article)) {
  if (!Array.isArray(uris) || uris.length === 0) {
    throw new Error(`Invalid article_ontology.json: by_article.${slug} must be a non-empty array`);
  }
  for (const uri of uris) {
    if (typeof uri !== 'string' || !uri.startsWith('lupine-research://')) {
      throw new Error(`Invalid article_ontology.json: by_article.${slug} contains non-canonical URI: ${uri}`);
    }
  }
}
...
// Article-level ontology reference footer. Reads ARTICLE_ONTOLOGY.by_article
// (populated by scripts/build-ontology-crosslinks.mjs); emits nothing for
// articles without declared nodes. Each link points to /atlas/#<short-id> so
// the public surface stays one hop from the canonical URI. Short IDs are
// used in the visible label; the full lupine-research:// URI is preserved
// as a title attribute for copy-paste and grep.
function articleOntology(slug) {
  const uris = ARTICLE_ONTOLOGY.by_article[slug];
  ...
  <p class="article-ontology-kicker">Ontology reference</p>
  <ul class="article-ontology-list">
${items}
  </ul>
  <p class="article-ontology-source">Source: <a href="/atlas/">Lupine research ontology</a> (lupine-research sphere, ${uris.length} node${uris.length === 1 ? '' : 's'}).</p>
</aside>`;
}
...
async function buildArticle(raw, slug) {
  ...
  // Ontology reference footer. Lives immediately after the share bar (or at
  // the end of the body if there is no share bar) and before the closing
  // colophon. Empty string for articles without declared nodes — see
  // ARTICLE_ONTOLOGY validation at the top of this file.
  const ontologyFooter = articleOntology(slug);
  if (ontologyFooter) {
    // Insert just before the </article> closing tag, which is the outermost
    // wrap around the article body. If we can't find it for any reason,
    // append to the end of the body — a footer at the end is still correct,
    // just slightly less visually anchored.
    if (body.includes('</article>')) {
      body = body.replace('</article>', `${ontologyFooter}\n</article>`);
    } else {
      body += '\n' + ontologyFooter;
    }
  }
