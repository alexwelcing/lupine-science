#!/usr/bin/env python3
"""Build a standalone HTML library page for the entire asset corpus."""
...
OUT = PROJECT / "renders" / "library.html"
...
def parse(name: str):
    ...
    path = f"assets/images/{folder + '/' if folder else ''}{name}"
    return {
        "filename": path,
        "motif": motif_name,
        "variant": variant,
...
<head>
  ...
  <title>Lupine Science — Brand Asset Library</title>
...
<body>
  <header>
    ...
    <nav class="nav">
      <a href="http://aledev.taild6f8cb.ts.net:8787/">App</a>
      <a href="https://lupine.science">lupine.science</a>
    </nav>
  </header>
  <main>
    <h1>Brand asset <em>library.</em></h1>
    <p class="lede">{len(images)} generated stills across all versions: research motifs, abstract textures, and standalone iconography. Use the app at <a href="http://aledev.taild6f8cb.ts.net:8787/">aledev.taild6f8cb.ts.net:8787</a> to search, filter, favorite, and download.</p>
    <div class="grid">
      {"\n".join(cards)}
    </div>
