#!/usr/bin/env python3
"""Generate the durable decoded-frame review package for t_fee03953."""
...
MASTER = ROOT / "renders" / "beyond-carbon-error-geometry-v3-review-1080p.mp4"
...
def main() -> None:
    ...
    add(samples, 0.5, "opening continuity bracket")
    add(samples, 1.0, "opening continuity bracket")
    add(samples, 2.0, "opening continuity bracket")
    ...
    for page_index in range(0, len(rows), 9):
        ...
        inputs = [str(ROOT / row["filename"].replace("decoded-frames/", "evidence/t_fee03953-v3/decoded-frames/")) for row in page]
