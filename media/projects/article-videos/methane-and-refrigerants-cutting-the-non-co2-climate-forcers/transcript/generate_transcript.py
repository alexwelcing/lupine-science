...
audio = Path(sys.argv[2]) if len(sys.argv) > 2 else project / "audio" / "narration-final.wav"
approved_file = Path(sys.argv[3]) if len(sys.argv) > 3 else (project / "narration.txt")
approved = approved_file.read_text(encoding="utf-8").strip()
raw_vtt = Path(sys.argv[4]) if len(sys.argv) > 4 else project / "audio" / "narration-raw.vtt"
