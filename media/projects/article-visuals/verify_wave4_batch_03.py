#!/usr/bin/env python3
"""Render and mechanically certify deterministic Wave-4 batch 03 assets."""
...
from wave4_batch_03_scenes import OUTPUT_DIR, RENDERERS, SPECS, render
...
OUTPUT_MANIFEST = OUTPUT_DIR / "batch-03-manifest.json"
SCENE_SOURCE = Path(__file__).with_name("wave4_batch_03_scenes.py")
...
def main() -> None:
    ...
    source_code = SCENE_SOURCE.read_text()
    forbidden_tokens = (
        "ImageFont", ".text(", ".multiline_text(", "openai", "anthropic",
        "requests", "httpx", "urllib", "subprocess", "arrow", "chart",
    )
    for token in forbidden_tokens:
        if token in source_code:
            errors.append(f"scene source contains forbidden operation/token: {token}")
