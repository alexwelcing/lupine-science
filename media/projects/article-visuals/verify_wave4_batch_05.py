#!/usr/bin/env python3
"""Render and mechanically certify deterministic Wave-4 batch 05 assets."""
...
from wave4_batch_05_scenes import OUTPUT_DIR, RENDERERS, SPECS, render
...
OUTPUT_MANIFEST = OUTPUT_DIR / "batch-05-manifest.json"
SCENE_SOURCE = Path(__file__).with_name("wave4_batch_05_scenes.py")
...
def forbidden_source_operations(tree: ast.Module) -> list[str]:
    ...
    errors: list[str] = []
    forbidden_calls = {"text", "multiline_text", "regular_polygon", "pieslice", "arc"}
...
def main() -> None:
    ...
    for asset_id, spec in SPECS.items():
        ...
        assets.append({
            ...
            "palette_histogram": {PAPER: counts[paper], INK: counts[ink], INDIGO: counts[indigo]},
