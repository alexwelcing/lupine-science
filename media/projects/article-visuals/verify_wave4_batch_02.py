#!/usr/bin/env python3
"""Render and mechanically certify deterministic Wave-4 batch 02 assets."""
...
from wave4_batch_02_scenes import OUTPUT_DIR, RENDERERS, SPECS, render
...
OUTPUT_MANIFEST = OUTPUT_DIR / "batch-02-manifest.json"
SCENE_SOURCE = Path(__file__).with_name("wave4_batch_02_scenes.py")
...
def functional_path_counts() -> dict[str, int]:
...
def source_has_forbidden_operations(path: Path) -> list[str]:
    """Reject font/text/network/process imports and drawing operations."""
    tree = ast.parse(path.read_text())
    errors: list[str] = []
    forbidden_imports = {
        "anthropic", "httpx", "openai", "requests", "subprocess", "urllib"
    }
    forbidden_calls = {"text", "multiline_text", "regular_polygon"}
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name.split(".")[0] in forbidden_imports or alias.name == "PIL.ImageFont":
                    errors.append(f"forbidden import {alias.name}")
        elif isinstance(node, ast.ImportFrom):
            module = node.module or ""
            if module.split(".")[0] in forbidden_imports or module == "PIL.ImageFont":
                errors.append(f"forbidden import {module}")
            if module == "PIL" and any(alias.name == "ImageFont" for alias in node.names):
                errors.append("forbidden import PIL.ImageFont")
        elif isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
            if node.func.attr in forbidden_calls:
                errors.append(f"forbidden drawing operation {node.func.attr}")
    return errors
...
def main() -> None:
    ...
    path_counts = functional_path_counts()
    ...
    errors.extend(f"scene source: {error}" for error in source_has_forbidden_operations(SCENE_SOURCE))
    ...
    for asset_id, spec in SPECS.items():
        ...
        path_count = path_counts.get(renderer_name, 0)
        if path_count != 1:
            errors.append(f"{asset_id}: expected exactly one functional path, found {path_count}")

        generator = (
            PROJECT / "media/projects/article-visuals"
        ...
        if not generator.is_file():
            ...
            continue
        errors.extend(
            f"{asset_id} generator: {error}"
            for error in source_has_forbidden_operations(generator)
        )
        ...
        if image_format != "PNG" or mode != "RGB":
            errors.append(f"{asset_id}: expected PNG/RGB, found {image_format}/{mode}")
        if colors != allowed:
        ...
        assets.append({
            ...
            "functional_path_count": path_count,
            "glyph_risk_control": (
                "PIL primitives only; no font import, text drawing, labels, glyphs, arrows, "
                "or chart marks"
            ),
    ...
    output_hashes = [item["output_sha256"] for item in assets]
    if len(set(output_hashes)) != len(output_hashes):
        errors.append("output SHA-256 values are not unique")
