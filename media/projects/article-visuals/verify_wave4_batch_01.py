#!/usr/bin/env python3
"""Render and mechanically certify deterministic Wave-4 batch 01 assets."""
...
from wave4_batch_01_scenes import OUTPUT_DIR, RENDERERS, SPECS, render
...
OUTPUT_MANIFEST = OUTPUT_DIR / "batch-01-manifest.json"
SCENE_SOURCE = Path(__file__).with_name("wave4_batch_01_scenes.py")
...
def main() -> None:
    ...
    forbidden_tokens = ("ImageFont", ".text(", ".multiline_text(")
    for token in forbidden_tokens:
        if token in source_code:
            errors.append(f"scene source contains forbidden text/font operation: {token}")
    ...
    for asset_id, spec in SPECS.items():
        ...
        generator = (
            PROJECT
            / "media/projects/article-visuals"
        ...
        if colors != allowed:
            errors.append(
                f"{asset_id}: expected exact three-color palette, found {sorted(colors)}"
            )
        if not top_open:
        ...
        assets.append(
            {
                ...
                "glyph_risk_control": (
                    "PIL primitives only; no font import, text drawing, labels, glyphs, "
                    "arrows, or chart marks"
                ),
